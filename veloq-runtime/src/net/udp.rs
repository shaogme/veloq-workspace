...
impl<S: OpSubmitter, P: SocketTokenPtr> GenericUdpSocket<S, P> {
...
impl UdpSocket {
    ...
    pub async fn send_to(
        ...
        if crate::runtime::context::try_current()
            .is_some_and(|ctx| ctx.handle.id() != self.inner.owner_worker_id())
        {
            let this = self.clone();
            return crate::runtime::context::spawn_to(
                self.inner.owner_worker_id(),
                move || async move { this.send_to_direct(buf, target).await },
            )
            .await;
        }
    ...
    pub async fn recv_stream(&self, buf: FixedBuf) -> io::Result<UdpRecvPacket> {
            ...
            .is_some_and(|ctx| ctx.handle.id() != self.inner.owner_worker_id())
        {
            ...
            return crate::runtime::context::spawn_to(
                self.inner.owner_worker_id(),
                move || async move { this.recv_stream_direct(buf).await },
            )
            .await;
        }
    ...
    pub async fn connect(&self, addr: SocketAddr) -> io::Result<()> {
            ...
            .is_some_and(|ctx| ctx.handle.id() != self.inner.owner_worker_id())
        {
            ...
            return crate::runtime::context::spawn_to(
                self.inner.owner_worker_id(),
                move || async move { this.connect_direct(addr).await },
            )
            .await;
        }
    ...
    pub async fn send_subset(
            ...
            .is_some_and(|ctx| ctx.handle.id() != self.inner.owner_worker_id())
        {
            ...
            return crate::runtime::context::spawn_to(
                self.inner.owner_worker_id(),
                move || async move { this.send_subset_direct(buf, buf_offset).await },
            )
            .await;
        }
    ...
    pub async fn recv_subset(
            ...
            .is_some_and(|ctx| ctx.handle.id() != self.inner.owner_worker_id())
        {
            ...
            return crate::runtime::context::spawn_to(
                self.inner.owner_worker_id(),
                move || async move { this.recv_subset_direct(buf, buf_offset).await },
            )
            .await;
        }
