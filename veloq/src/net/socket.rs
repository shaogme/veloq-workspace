...
impl TcpSocket {
    ...
    pub fn listen<'a>(self, ctx: &'a RuntimeContext, backlog: u32) -> VeloqResult<TcpListener<'a>> {
...
impl UdpSocketBuilder {
    ...
    pub fn bind<'a, A: ToSocketAddrs>(
        self,
        ctx: &'a RuntimeContext,
        addr: A,
    ) -> VeloqResult<UdpSocket<'a>> {
        let addr = addr
            .to_socket_addrs()
            .map_err(from_io_error)?
            .next()
