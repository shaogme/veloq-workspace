...
use veloq_std::{cell::RefCell, error::Error, fmt, ops::AsyncFnOnce, pin::pin, rc::Rc};
...
impl<T> Inner<T> {
    ...
    fn new(init: T) -> Self {
        Self {
            state: RefCell::new(WatchState::new(init, 1)),
    ...
    ///
    /// The closure runs while the channel state is mutably borrowed. It must
    /// not re-enter this channel through sending, subscribing, receiver
    /// counting, borrowing, or any endpoint operation.
    ...
    pub fn receiver_count(&self) -> usize {
        self.state.borrow().receiver_count
    }
...
///
/// This borrow protects the complete channel state. Holding it can delay
/// sends, subscriptions, receiver destruction, and sender shutdown.
pub struct Ref<'a, T> {
    inner: CoreRef<'a, WatchState<T>>,
}
...
impl<'a, T> BorrowedSender<'a, T> {
    ...
    /// Modifies the value in place using the given closure and notifies receivers.
    ///
    /// The closure runs while the channel state is mutably borrowed and must
    /// not re-enter this channel.
    pub fn send_modify<R>(&self, modify: impl FnOnce(&mut T) -> R) -> R {
...
pub fn channel<T>(init: T) -> (Sender<T>, Receiver<T>) {
    let state = Rc::new(Inner::new(init));
