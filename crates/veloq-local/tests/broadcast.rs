...
#[tokio::test]
async fn test_local_broadcast_multi_receiver() {
...
#[tokio::test]
async fn test_local_broadcast_lagged() {
...
#[tokio::test]
async fn test_local_broadcast_try_recv() {
    let (tx, mut rx) = broadcast::channel(2);
...
#[tokio::test]
async fn test_local_broadcast_sender_drop() {
    let (tx, mut rx) = broadcast::channel(16);
    tx.send(100).unwrap();
    tx.send(200).unwrap();
    drop(tx);
...
#[tokio::test]
async fn test_local_broadcast_no_receiver() {
...
#[tokio::test]
async fn test_local_broadcast_resubscribe_and_clone() {
    let (tx, mut rx1) = broadcast::channel(16);
    tx.send(1).unwrap();
    tx.send(2).unwrap();
...
#[tokio::test]
async fn test_local_broadcast_concurrent() {
    ...
    local
        .run_until(async {
            ...
            let receiver1_handle = spawn_local(async move {
                let mut count = 0;
                while count < 50 {
            ...
            let receiver2_handle = spawn_local(async move {
                let mut count = 0;
                while count < 50 {
...
#[tokio::test]
async fn test_local_broadcast_cancellation() {
    let local = LocalSet::new();
