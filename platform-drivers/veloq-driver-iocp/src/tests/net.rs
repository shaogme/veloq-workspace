...
use veloq_driver_core::op::{
    Accept as AcceptBase, Connect as ConnectBase, Recv as RecvBase, SendTo as SendToBase,
    UdpRecvStream as UdpRecvStreamBase, UdpRefill as UdpRefillBase,
};
...
fn test_iocp_accept() {
    ...
    let op = <Accept as veloq_driver_core::op::IntoPlatformOp<IocpOp>>::from_user_payload(
        accept_payload
            .take()
...
fn test_iocp_recv_with_buffer_pool() {
    ...
    let mut op = <Recv as veloq_driver_core::op::IntoPlatformOp<IocpOp>>::from_user_payload(
        recv_payload
            .take()
...
fn test_rio_udp_send_to_recv_from_address_path() {
    ...
    let recv_out =
        <UdpRecvStream as veloq_driver_core::op::IntoPlatformOp<IocpOp>>::from_user_payload(
            recv_payload
                .take()
...
fn test_rio_udp_recv_pool_burst_waiters_raise_target() {
    ...
    for (ud, generation, recv_payload) in submitted {
        driver.cancel_op(ud);
        let _ = <UdpRecvStream as veloq_driver_core::op::IntoPlatformOp<IocpOp>>::from_user_payload(
            recv_payload,
        );
        let res = wait_completion(
            &mut driver,
