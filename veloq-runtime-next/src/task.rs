...
#[macro_export]
macro_rules! spawn {
    ($scope:ident, $task_name:expr) => {
        $scope.spawn(&$task_name)
    };
}

#[macro_export]
macro_rules! spawn_boxed {
    ($scope:ident, $future_expr:expr) => {
        $scope.spawn_boxed($future_expr)
    };
}

#[macro_export]
macro_rules! spawn_to {
    ($scope:ident, $worker_id:expr, $task_name:expr) => {
        $scope.spawn_to($worker_id, &$task_name)
    };
}

#[macro_export]
macro_rules! spawn_boxed_to {
    ($scope:ident, $worker_id:expr, $future_expr:expr) => {
        $scope.spawn_boxed_to($worker_id, $future_expr)
    };
}

#[macro_export]
macro_rules! spawn_boxed_local {
    ($scope:ident, $future_expr:expr) => {
        $scope.__private_push_boxed_local($future_expr)
    };
}

#[macro_export]
macro_rules! spawn_local {
    ($scope:ident, $handle_name:ident, $task_node_ref:expr) => {
        let $handle_name = $scope.__private_push_local(&$task_node_ref);
    };
}
